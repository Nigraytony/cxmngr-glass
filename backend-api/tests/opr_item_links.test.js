...
describe('OPR item linking (issues + functional tests)', function () {
  ...
  async function createOprItems({ token, projectId }) {
    ...
    const qRes = await withCsrf(request(app)
    ...
    const openRes = await withCsrf(request(app)
    ...
    for (let i = 1; i <= 5; i += 1) {
      // eslint-disable-next-line no-await-in-loop
      const aRes = await withCsrf(request(app)
  ...
  it('allows linking active OPR items to an issue and functional test', async () => {
    ...
    const issueRes = await withCsrf(request(app)
      .post('/api/issues'))
      .set('Authorization', `Bearer ${token}`)
    ...
    const patchRes = await withCsrf(request(app)
