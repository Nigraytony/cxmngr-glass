    ...
    <div class="w-full rounded-2xl p-4 md:p-6 bg-white/6 backdrop-blur-xl border border-white/10">
      ...
      <div>
        ...
        <div v-if="currentTab === 'Info'" class="grid md:grid-cols-2 gap-x-4 gap-y-2 items-start">
          ...
          <div>
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                ...
                <input v-model.number="form.number" type="number" min="0" :disabled="isClosed" class="w-full px-3 py-2 rounded-md bg-white/10 border border-white/20 placeholder-gray-400 disabled:opacity-60" placeholder="e.g. 101" />
              </div>
              <div>
                <label class="block text-sm text-white/70">Type</label>
                <select v-model="form.type" :disabled="isClosed" class="w-full px-3 py-2 rounded-md bg-white/10 border border-white/20 disabled:opacity-60">
                  <option v-for="opt in issueTypeOptions" :key="String(opt.value)" :value="opt.value">{{ opt.text }}</option>
                </select>
              </div>
            </div>
            ...
            <div class="mt-2">
              ...
              <input v-model="form.title" type="text" :disabled="isClosed" class="w-full px-3 py-2 rounded-md bg-white/10 border border-white/20 placeholder-gray-400 disabled:opacity-60" placeholder="Issue title" />
            </div>
            ...
            <div class="grid grid-cols-2 gap-3 mt-2">
              <div>
                ...
                <select v-model="form.priority" :disabled="isClosed" class="w-full px-3 py-2 rounded-md bg-white/10 border border-white/20 disabled:opacity-60">
                  <option v-for="opt in priorityOptions" :key="String(opt.value)" :value="opt.value">{{ opt.text }}</option>
                </select>
              </div>
              <div>
                <label class="block text-sm text-white/70">Status</label>
                <select v-model="form.status" :disabled="isClosed" class="w-full px-3 py-2 rounded-md bg-white/10 border border-white/20 disabled:opacity-60">
                  <option v-for="opt in statusOptions" :key="String(opt.value)" :value="opt.value">{{ opt.text }}</option>
                </select>
              </div>
            </div>

            <!-- Closed By / Closed Date row (visible when closed) -->
            <div v-if="isClosed" class="grid grid-cols-2 gap-3 mt-2">
              <div>
                <label class="block text-sm text-white/70">Closed By</label>
                <input v-model="form.closedBy" type="text" disabled class="w-full px-3 py-2 rounded-md bg-white/10 border border-white/20 placeholder-gray-400 disabled:opacity-60" />
              </div>
              <div>
                <label class="block text-sm text-white/70">Closed Date</label>
                <input v-model="form.closedDate" type="date" class="w-full px-3 py-2 rounded-md bg-white/10 border border-white/20 placeholder-gray-400" />
              </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3 mt-2">
              <div>
                ...
                <input v-model="form.foundBy" type="text" :disabled="isClosed" class="w-full px-3 py-2 rounded-md bg-white/10 border border-white/20 placeholder-gray-400 disabled:opacity-60" placeholder="Name or team" />
              </div>
              <div>
                <label class="block text-sm text-white/70">Date Found</label>
                <input v-model="form.dateFound" type="date" :disabled="isClosed" class="w-full px-3 py-2 rounded-md bg-white/10 border border-white/20 disabled:opacity-60" />
              </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3 mt-2">
              <div>
                ...
                <input v-model="form.location" type="text" :disabled="isClosed" class="w-full px-3 py-2 rounded-md bg-white/10 border border-white/20 placeholder-gray-400 disabled:opacity-60" placeholder="e.g. Lobby, Level 2" />
              </div>
              <div>
                <label class="block text-sm text-white/70">System</label>
                <input v-model="form.system" type="text" :disabled="isClosed" class="w-full px-3 py-2 rounded-md bg-white/10 border border-white/20 placeholder-gray-400 disabled:opacity-60" placeholder="e.g. HVAC, Lighting" />
              </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3 mt-2">
              <div>
                <div class="flex items-center justify-between">
                  ...
                  <button type="button" class="text-xs px-2 py-1 rounded bg-white/10 border border-white/20 hover:bg-white/15 disabled:opacity-60 disabled:cursor-not-allowed" :disabled="isClosed" @click="assignToMe" v-if="assignToMeLabel">Assign to me</button>
                </div>
                <input v-model="form.assignedTo" type="text" :disabled="isClosed" class="w-full px-3 py-2 rounded-md bg-white/10 border border-white/20 placeholder-gray-400 disabled:opacity-60" placeholder="Name / email" :list="assignedListId" />
                <datalist :id="assignedListId">
                  <option v-for="m in assignedToSuggestions" :key="m" :value="m">{{ m }}</option>
                </datalist>
              </div>
              <div>
                <label class="block text-sm text-white/70">Due Date</label>
                <input v-model="form.dueDate" type="date" :disabled="isClosed" class="w-full px-3 py-2 rounded-md bg-white/10 border border-white/20 disabled:opacity-60" />
              </div>
            </div>
          ...
          <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-3 mt-2">
            <div>
              ...
              <textarea v-model="form.recommendation" rows="3" :disabled="isClosed" class="w-full px-3 py-2 rounded-md bg-white/10 border border-white/20 placeholder-gray-400 disabled:opacity-60" placeholder="Recommendation"></textarea>
            </div>
            <div>
              <label class="block text-sm text-white/70">Resolution</label>
              <textarea v-model="form.resolution" rows="3" :disabled="isClosed" class="w-full px-3 py-2 rounded-md bg-white/10 border border-white/20 placeholder-gray-400 disabled:opacity-60" placeholder="Resolution"></textarea>
            </div>
          </div>
          ...
          <div class="md:col-span-2">
            <div class="flex items-center gap-3 mt-2 justify-end md:justify-start">
              ...
              <button @click="goPrevIssue" :disabled="!prevIssueId" class="px-3 py-2 rounded-md bg-white/10 border border-white/20 hover:bg-white/15 inline-flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed" title="Previous issue">
              ...
              <button @click="goNextIssue" :disabled="!nextIssueId" class="px-3 py-2 rounded-md bg-white/10 border border-white/20 hover:bg-white/15 inline-flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed" title="Next issue">
                <span>Next</span>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" class="w-4 h-4">
              ...
              <div class="inline-flex items-center gap-2 ml-2 mr-1">
                ...
                <span class="text-xs select-none" :class="isClosedToggle ? 'text-emerald-200' : 'text-white/60'">{{ isClosedToggle ? 'YES' : 'NO' }}</span>
              </div>
              <button @click="save()" :disabled="saving || isClosed" class="px-3 py-2 rounded-md bg-white/20 border border-white/30 hover:bg-white/30 inline-flex items-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed" :class="saving || isClosed ? 'opacity-60 cursor-not-allowed' : ''">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" class="w-4 h-4">
                  <path d="M5 13l4 4L19 7" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
                <span>Save</span>
              </button>
              <button @click="downloadIssuePdf" class="px-3 py-2 rounded-md bg-white/10 border border-white/20 hover:bg-white/15 inline-flex items-center gap-2" title="Download PDF report">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" class="w-4 h-4">
                  ...
                  <rect x="4" y="18" width="16" height="2" rx="1" stroke-width="1.5"/>
                </svg>
          ...
          <div>
            ...
            <div class="mt-2 flex flex-wrap gap-2">
              ...
              >
                ...
                <button class="absolute bottom-1.5 right-1.5 z-10 h-7 w-7 grid place-items-center rounded-md bg-black/60 hover:bg-black/75 border border-white/20 text-white/90 focus:outline-none focus:ring-2 focus:ring-white/40 disabled:opacity-50 disabled:cursor-not-allowed"
                        title="Delete photo"
                ...
                <button class="relative w-full h-full focus:outline-none focus:ring-2 focus:ring-white/40" @click="openViewer(idx)">
                  <img :src="p?.data || p" class="w-full h-full object-cover" />
        ...
        <div v-else-if="currentTab === 'Comments'" class="space-y-4">
          <div :class="isClosed ? 'opacity-60 pointer-events-none' : ''" aria-disabled="true">
            <Comments
              :model-value="form.comments"
              @update:modelValue="(v:any) => { if (!isClosed) form.comments = v }"
              :onAdd="isClosed ? undefined : onAddComment"
              :onDelete="isClosed ? undefined : onDeleteComment"
            />
          </div>
        </div>
          ...
          <!-- Manual link add (optional) -->
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3 items-end">
              <div>
                ...
                <input v-model="newAttachment.filename" type="text" :disabled="isClosed" class="w-full px-3 py-2 rounded-md bg-white/10 border border-white/20 placeholder-gray-400 disabled:opacity-60" placeholder="specs.pdf" />
              </div>
              <div>
                <label class="block text-sm text-white/70">URL</label>
                <input v-model="newAttachment.url" type="url" :disabled="isClosed" class="w-full px-3 py-2 rounded-md bg-white/10 border border-white/20 placeholder-gray-400 disabled:opacity-60" placeholder="https://..." />
              </div>
              <div class="flex items-center gap-2">
                <button @click="addAttachment" :disabled="isClosed" class="px-3 py-2 rounded-md bg-white/20 border border-white/30 hover:bg-white/30 disabled:opacity-60 disabled:cursor-not-allowed">Add</button>
              </div>
            </div>
          <div>
            ...
            <ul class="space-y-2">
              <li v-for="(a, i) in form.attachments" :key="i" class="p-2 rounded-md bg-white/5 border border-white/10 flex items-center justify-between gap-3">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <button @click="removeAttachment(i)" :disabled="isClosed" class="h-8 w-8 grid place-items-center rounded-md bg-red-500/20 border-2 border-red-400/80 text-red-200 hover:bg-red-500/50 hover:border-red-500/90 hover:text-red-100 focus:outline-none focus:ring-2 focus:ring-red-400/60 transition-colors duration-150 disabled:opacity-50 disabled:cursor-not-allowed" title="Remove" aria-label="Remove">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" class="w-4 h-4">
                      <path d="M3 6h18" stroke-width="1.5" stroke-linecap="round"/>
                      <path d="M8 6l1-2h6l1 2" stroke-width="1.5" stroke-linecap="round"/>
                      <rect x="6" y="6" width="12" height="14" rx="1.5" stroke-width="1.5"/>
    ...
    <Modal v-model="viewerOpen">
      <template #header>
        <div class="flex items-center justify-between w-full">
          <div class="font-medium">Photo {{ (photos && photos.length) ? (viewerIndex + 1) : 0 }} / {{ (photos && photos.length) || 0 }}</div>
          <div class="text-white/70 text-sm">Press Esc or click outside to close</div>
        </div>
      </template>
      <div class="flex items-center justify-center">
        <div v-if="photos && photos.length" class="max-h-[70vh] max-w-full overflow-auto rounded-md border border-white/10 bg-black/20">
          <img :src="(photos[viewerIndex]?.data || photos[viewerIndex])" class="block select-none" :style="{ transform: `rotate(${rotation}deg) scale(${zoom})`, transformOrigin: 'center center' }" />
        </div>
      ...
      <template #footer>
        <div class="flex flex-wrap items-center justify-between gap-3 w-full">
          ...
          <div class="flex items-center gap-2">
            ...
            <button @click="removePhotoAt(viewerIndex)" :disabled="isClosed" class="px-3 py-2 rounded-md bg-red-500/20 border border-red-400/40 hover:bg-red-500/30 text-red-200 disabled:opacity-50 disabled:cursor-not-allowed">Delete</button>
...
watch(pageTitle, (t) => { try { document.title = t } catch {} }, { immediate: true })
onBeforeUnmount(() => { try { document.title = prevDocTitle.value } catch {} })
...
onMounted(async () => {
  ...
  try { await issues.fetchIssues(pid) } catch {}
  if (!isNew.value) {
    try {
