<template>
  <section class="space-y-6 relative" ref="pageSection">
    ...
    <div class="flex items-center gap-2">
      ...
      <input v-model="search" type="text" placeholder="Search by tag, title or type" class="px-3 py-2 rounded bg-white/10 border border-white/20 text-white placeholder-white/50 w-64" />
      <div class="flex items-center gap-2">
            ...
            <div class="py-1">
              <button
                ...
                <span class="inline-flex items-center gap-2">
                  <span class="inline-block w-2.5 h-2.5 rounded-full" :class="statusDotClass(opt.name)"></span>
                  <span>{{ opt.name }}</span>
                </span>
    ...
    <div class="rounded-xl border border-white/10 bg-white/5 backdrop-blur p-2">
      ...
      <div class="divide-y divide-white/10">
        <div v-for="t in paged" :key="t.id" class="grid grid-cols-12 items-center px-2 py-2 text-white/90">
          <div class="col-span-1 truncate" :title="t.tag">{{ t.tag || '-' }}</div>
          <div class="col-span-3 truncate" :title="t.title">{{ t.title }}</div>
          <div class="col-span-2 truncate">{{ t.type }}</div>
          <div class="col-span-2 truncate">{{ t.system || '-' }}</div>
          <div class="col-span-2 truncate" :title="spaceName(t.spaceId)">{{ spaceName(t.spaceId) || '-' }}</div>
          <div class="col-span-1 truncate">{{ t.status || '-' }}</div>
          <div class="col-span-1 flex items-center justify-end gap-2">
            ...
            <button @click="duplicateTemplate(t)" class="w-8 h-8 grid place-items-center rounded-lg bg-white/6 hover:bg-white/10 text-white border border-white/10" aria-label="Duplicate" title="Duplicate">
            ...
            <button @click="openEdit(t)" class="w-8 h-8 grid place-items-center rounded-lg bg-white/6 hover:bg-white/10 text-white border border-white/10" aria-label="Edit">
            ...
            <button @click="confirmRemove(t)" class="w-8 h-8 grid place-items-center rounded-lg bg-red-500/15 hover:bg-red-500/25 text-red-200 border border-red-500/30" aria-label="Delete">
        ...
        <div v-if="!templates.length && !loading" class="p-6 text-white/60 text-center">No templates yet.</div>
        <div v-else-if="!filtered.length && !loading" class="p-6 text-white/60 text-center">No matching templates.</div>
    ...
    <div v-if="modalOpen" class="fixed inset-0 z-50 grid place-items-center bg-black/50">
      <div class="w-[640px] max-w-[95vw] rounded-xl border border-white/20 bg-white/10 backdrop-blur p-4 text-white">
        <div class="flex items-center justify-between mb-3">
          <h2 class="text-lg font-semibold">{{ editing ? 'Edit Template' : 'Create Template' }}</h2>
