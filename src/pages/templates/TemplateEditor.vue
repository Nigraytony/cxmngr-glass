<template>
  ...
  <!-- Tabs header: Info, Components, Photos, Attachments, Checklists, FPT, Instances -->
      <div class="mb-4 md:mb-6">
        <div role="tablist" class="relative flex items-center w-full">
          ...
          <button v-for="t in tabs" :key="t" @click="currentTab = t" :aria-selected="currentTab === t" role="tab" class="flex-1 text-center px-3 py-2 text-sm flex items-center justify-center gap-2" :class="currentTab === t ? 'text-white border-b-2 border-white rounded-t-md bg-white/6' : 'text-white/70 hover:text-white/90'">
            ...
            <svg v-else-if="t === 'Components'" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor"><rect x="3" y="3" width="8" height="8" rx="1.5" stroke-width="1.5"/><rect x="13" y="3" width="8" height="8" rx="1.5" stroke-width="1.5"/><rect x="3" y="13" width="8" height="8" rx="1.5" stroke-width="1.5"/><rect x="13" y="13" width="8" height="8" rx="1.5" stroke-width="1.5"/></svg>
            ...
            <svg v-else-if="t === 'Instances'" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor"><rect x="3" y="4" width="18" height="16" rx="2" stroke-width="1.5"/><path d="M7 8h10M7 12h6M7 16h8" stroke-width="1.5"/></svg>
            <span>{{ t }}</span>
      ...
      <div>
          ...
          <div>
            <div class="grid grid-cols-2 gap-3">
              <div>
                ...
                <div v-if="!isValidTag" class="text-xs text-red-300 mt-1">Tag is required.</div>
              </div>
              <div>
                ...
                <div v-if="!isValidType" class="text-xs text-red-300 mt-1">Type is required.</div>
              </div>
            </div>
            <div class="mt-2">
              ...
              <div v-if="!isValidTitle" class="text-xs text-red-300 mt-1">Title is required.</div>
            </div>
            ...
            <div class="mt-2">
              ...
              <div class="relative">
                <select v-model="(form as any).spaceId" class="w-full px-3 pr-10 py-2 rounded-md bg-white/10 border border-white/20 appearance-none">
            ...
            <div class="mt-4 flex items-center gap-2">
              <button @click="save" :disabled="saving || !isValidForm" class="px-3 py-2 rounded-md bg-white/20 border border-white/30 hover:bg-white/30 inline-flex items-center gap-2" :class="(saving || !isValidForm) ? 'opacity-60 cursor-not-allowed' : ''">
              ...
              <button @click="onDelete" class="px-3 py-2 rounded-md bg-red-500/20 border border-red-500/40 text-red-200 hover:bg-red-500/30 inline-flex items-center gap-2">
          ...
          <div class="space-y-2">
            <div class="rounded-md border border-white/10 bg-white/5 p-3">
              <div class="flex items-center justify-between mb-2">
                <div class="text-sm text-white/80">Attributes</div>
                <div class="text-xs text-white/60">{{ currentAttributes.length }} items</div>
              </div>
              <div v-if="currentAttributes.length === 0" class="text-white/60 text-sm">No attributes yet.</div>
              <ul v-else class="divide-y divide-white/10">
                <li v-for="(a, i) in currentAttributes" :key="i" class="py-2 flex items-center gap-2">
                  <template v-if="editingIndex === i">
                    <input v-model="editKey" class="w-40 px-2 py-1 rounded-md bg-white/10 border border-white/20" placeholder="Key" />
                    <input v-model="editValue" class="flex-1 min-w-0 px-2 py-1 rounded-md bg-white/10 border border-white/20" placeholder="Value" />
                    <button @click="saveEdit(i)" class="px-2 py-1 rounded-md bg-emerald-500/20 border border-emerald-400/60 text-emerald-100 hover:bg-emerald-500/35 text-sm">Save</button>
                    <button @click="cancelEdit" class="px-2 py-1 rounded-md bg-white/10 border border-white/20 hover:bg-white/15 text-sm">Cancel</button>
                  </template>
                  <template v-else>
                    <span class="w-40 truncate text-white/90">{{ a.key }}</span>
                    <span class="flex-1 min-w-0 truncate text-white/70">{{ a.value }}</span>
                    <button @click="startEdit(i)" class="px-2 py-1 rounded-md bg-white/10 border border-white/20 hover:bg-white/15 text-sm">Edit</button>
                    <button @click="removeAttr(i)" class="px-2 py-1 rounded-md bg-red-500/20 border border-red-400/60 text-red-100 hover:bg-red-500/35 text-sm">Remove</button>
                  </template>
                </li>
              </ul>
              <div class="mt-3 flex items-center gap-2">
                <input v-model="newAttrKey" class="w-40 px-2 py-1 rounded-md bg-white/10 border border-white/20" placeholder="Key" />
                <input v-model="newAttrValue" class="flex-1 min-w-0 px-2 py-1 rounded-md bg-white/10 border border-white/20" placeholder="Value" />
                <button @click="addAttr" :disabled="!newAttrKey.trim()" class="px-2 py-1 rounded-md bg-white/10 border border-white/20 hover:bg-white/15 text-sm" :class="!newAttrKey.trim() ? 'opacity-60 cursor-not-allowed' : ''">Add</button>
              </div>
            </div>
        ...
        <!-- Photos Tab -->
        <div v-else-if="currentTab === 'Photos'" class="space-y-3">
          <div class="flex items-center justify-between">
            <div class="text-white/80">Photos</div>
            <PhotoUploader
              :upload="uploadPhoto"
          ...
          <div v-if="!(form as any).photos || !(form as any).photos.length" class="text-white/60 text-sm">No photos yet.</div>
          <div v-else class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            <div v-for="(p, idx) in (form as any).photos" :key="idx" class="rounded-md overflow-hidden border border-white/10 bg-white/5">
              <img :src="p.data" :alt="p.filename" class="w-full h-40 object-cover" />
              <div class="p-2 text-xs text-white/70 flex items-center justify-between gap-2">
                <input v-model="p.caption" @change="updatePhotoMeta(idx, p.caption)" placeholder="Caption" class="w-full px-2 py-1 rounded bg-white/10 border border-white/20" />
                <button @click="removePhoto(idx)" class="ml-2 px-2 py-1 rounded bg-red-500/20 border border-red-500/40 text-red-200 hover:bg-red-500/30">Delete</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <!-- Attachments Tab -->
        <div v-else-if="currentTab === 'Attachments'" class="space-y-3">
          <div class="flex items-center justify-between">
            <div class="text-white/80">Attachments</div>
            <DocumentUploader
          ...
          <div v-if="!(form as any).attachments || !(form as any).attachments.length" class="text-white/60 text-sm">No attachments yet.</div>
          <div v-else class="space-y-2">
            <div v-for="(a, idx) in (form as any).attachments" :key="idx" class="flex items-center justify-between gap-3 p-2 rounded-md bg-white/5 border border-white/10">
              <div class="truncate">
                <a :href="a.url || a.data" target="_blank" rel="noopener" class="text-white/90 hover:underline">{{ a.filename || a.url }}</a>
              </div>
              <button @click="removeAttachment(idx)" class="px-2 py-1 rounded bg-red-500/20 border border-red-500/40 text-red-200 hover:bg-red-500/30 text-sm">Remove</button>
            </div>
          </div>
        </div>
        ...
        <div v-else-if="currentTab === 'Checklists'">
          <ChecklistPanel v-model="checklists" :project-id="String(form.projectId || '')" :equipment-id="String(form.id || (form as any)._id || '')" :equipment-tag="form.tag || ''" @change="onChecklistsChange" />
        </div>

        <!-- FPT Tab -->
        <div v-else-if="currentTab === 'FPT'">
          <FunctionalTestsPanel v-model="functionalTests" :project-id="String(form.projectId || '')" :equipment-id="String(form.id || (form as any)._id || '')" :equipment-tag="form.tag || ''" @change="onFunctionalTestsChange" @save="onFunctionalTestsSave" />
        </div>
        
        <!-- Instances Tab -->
        <div v-else-if="currentTab === 'Instances'" class="space-y-3">
          <div class="flex items-center justify-between">
            <div class="text-white/80">Equipment created from this template.</div>
            <div class="flex items-center gap-2">
              ...
              >
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" class="w-4 h-4"><path d="M4 12h16" stroke-width="1.5" stroke-linecap="round"/><path d="M10 6l6 6-6 6" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
              ...
              <button @click="showCreateDialog = true" class="px-3 py-2 rounded-md bg-white/10 border border-white/20 hover:bg-white/15 inline-flex items-center gap-2">
              ...
              <button @click="showUploadDialog = true" class="px-3 py-2 rounded-md bg-white/10 border border-white/20 hover:bg-white/15 inline-flex items-center gap-2">
          ...
          <div v-if="!instancesForTemplate.length" class="text-white/60">No equipment instances yet.</div>
          <div v-else class="overflow-x-auto rounded-md border border-white/10">
            <table class="min-w-full text-sm">
              <thead class="bg-white/5 text-white/70">
                <tr>
                  <th class="w-8 px-2 py-2"><input type="checkbox" :checked="allSelected" @change="toggleSelectAll(($event.target as HTMLInputElement)?.checked)" /></th>
              ...
              <tbody>
                <tr v-for="e in instancesForTemplate" :key="e.id" class="border-t border-white/10 hover:bg-white/5">
                  <td class="px-2 py-2 align-middle"><input type="checkbox" :value="e.id" v-model="selectedInstanceIds" /></td>
                  ...
                  <td class="px-3 py-2 align-middle text-right">
                    <RouterLink :to="{ name: 'equipment-edit', params: { id: e.id } }" class="h-8 w-8 inline-grid place-items-center rounded-md bg-white/10 border border-white/20 hover:bg-white/15" title="Open" aria-label="Open">
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" class="w-4 h-4">
                        <path d="M4 12h16" stroke-width="1.5" stroke-linecap="round"/>
                        <path d="M14 6l6 6-6 6" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                      </svg>
                    </RouterLink>
                  </td>
                </tr>
          ...
          <Modal v-model="showCreateDialog" :panel-class="'max-w-xl'">
            <template #header>
              <div class="flex items-center justify-between">
                <div class="text-lg">Create equipment from template</div>
              </div>
            </template>
            <div class="space-y-4">
              <div class="text-sm text-white/70">Provide tags for the new equipment. You can either paste a comma-separated list, or generate tags using a prefix and count. Location/space and status will be inherited from this template.</div>

              <div class="flex items-center gap-3">
                <button @click="createMode = 'csv'" class="px-3 py-1.5 rounded-md border" :class="createMode === 'csv' ? 'bg-white/20 border-white/40' : 'bg-white/10 border-white/20'">CSV list</button>
                <button @click="createMode = 'prefix'" class="px-3 py-1.5 rounded-md border" :class="createMode === 'prefix' ? 'bg-white/20 border-white/40' : 'bg-white/10 border-white/20'">Prefix + count</button>
              </div>

              <div v-if="createMode === 'csv'" class="space-y-2">
                <label class="block text-sm text-white/80">Tags (comma-separated)</label>
                <textarea v-model="createForm.tagsCsv" rows="3" placeholder="e.g. AHU-01, AHU-02, AHU-03" class="w-full px-3 py-2 rounded-md bg-white/10 border border-white/20"></textarea>
              </div>

              <div v-else class="grid grid-cols-2 gap-3">
                <div>
                  <label class="block text-sm text-white/80">Prefix</label>
                  <input v-model="createForm.prefix" placeholder="e.g. AHU-" class="w-full px-3 py-2 rounded-md bg-white/10 border border-white/20" />
                </div>
                <div>
                  <label class="block text-sm text-white/80">Count</label>
                  <input v-model.number="createForm.count" type="number" min="1" max="200" placeholder="e.g. 3" class="w-full px-3 py-2 rounded-md bg-white/10 border border-white/20" />
                </div>
              </div>

              <div class="rounded-md bg-white/5 border border-white/10 p-3">
                <div class="text-xs text-white/60 mb-1">Preview ({{ generatedTags.length }})</div>
                <div v-if="!generatedTags.length" class="text-white/50 text-sm">No tags yet.</div>
                <div v-else class="flex flex-wrap gap-2">
                  <span v-for="t in generatedTags.slice(0, 20)" :key="t" class="px-2 py-0.5 rounded bg-white/10 border border-white/20 text-xs">{{ t }}</span>
                  <span v-if="generatedTags.length > 20" class="text-xs text-white/60">+{{ generatedTags.length - 20 }} more…</span>
                </div>
              </div>

              <div v-if="createError" class="text-sm text-red-300">{{ createError }}</div>
            </div>
            <template #footer>
              <div class="flex items-center justify-between gap-3">
                <div v-if="duplicateTags.length" class="text-xs text-amber-200 bg-amber-500/10 border border-amber-400/30 rounded-md px-2 py-1">
                  <div class="flex items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" class="w-4 h-4"><path d="M12 3l9 16H3l9-16z" stroke-width="1.5" stroke-linejoin="round"/><path d="M12 9v4" stroke-width="1.5" stroke-linecap="round"/><path d="M12 17h.01" stroke-width="2" stroke-linecap="round"/></svg>
                ...
                <div class="flex items-center gap-2">
                  <button @click="showCreateDialog = false" class="px-3 py-2 rounded-md bg-white/10 border border-white/20 hover:bg-white/15">Cancel</button>
                  <button @click="submitCreate" :disabled="creating || generatedTags.length === 0" class="px-3 py-2 rounded-md border text-emerald-100 hover:bg-emerald-500/20" :class="[(creating || generatedTags.length === 0) ? 'opacity-60 cursor-not-allowed' : '', duplicateTags.length ? 'bg-amber-500/10 border-amber-400/40 text-amber-100 hover:bg-amber-500/20' : 'bg-emerald-500/20 border-emerald-400/50']">
          ...
          <!-- Upload tags dialog -->
          <Modal v-model="showUploadDialog" :panel-class="'max-w-2xl'">
            <template #header>
              <div class="flex items-center justify-between">
                <div class="text-lg">Upload tags from spreadsheet</div>
              </div>
            </template>
            <div class="space-y-4">
              <div class="text-sm text-white/70">Upload a CSV/XLSX with columns: <span class="font-medium">tag</span> (required), <span class="font-medium">title</span> (optional), <span class="font-medium">location</span> (optional), <span class="font-medium">status</span> (optional).</div>
              <div>
              ...
              <div v-if="uploadRows.length" class="rounded-md bg-white/5 border border-white/10">
                <div class="px-3 py-2 text-xs text-white/60 flex items-center justify-between">
                  ...
                  <div v-if="uploadInvalidRowsCount" class="text-red-200">Invalid: {{ uploadInvalidRowsCount }} (missing tag)</div>
                </div>
                  ...
                  <table class="min-w-full text-xs">
                    ...
                    <tbody>
                      <tr v-for="(r, i) in uploadRows.slice(0, 10)" :key="i" class="border-t border-white/10">
                        ...
                        <td class="px-2 py-1 truncate">{{ r.location }}</td>
      ...
      <div class="mt-6 pt-3 border-t border-white/10 flex items-center gap-2 justify-between">
        <div class="text-xs text-white/60">{{ positionText }}</div>
        <div class="flex items-center gap-2">
          <button @click="goPrev" :disabled="!prevTemplateId" class="px-3 py-1.5 rounded-md bg-white/10 border border-white/20 hover:bg-white/15 disabled:opacity-50">Prev</button>
          <button @click="goNext" :disabled="!nextTemplateId" class="px-3 py-1.5 rounded-md bg-white/10 border border-white/20 hover:bg-white/15 disabled:opacity-50">Next</button>
        </div>
      </div>
    </div>
  ...
  try {
    ...
    for (const eqId of selectedInstanceIds.value) {
      try {
        ...
        try { await (equipmentStore as any).updateFields(eqId, { attributes: base.attributes }) } catch {}
        try { await (equipmentStore as any).updateFields(eqId, { components: base.components }) } catch {}
        try { await (equipmentStore as any).updateFields(eqId, { checklists: base.checklists }) } catch {}
        try { await (equipmentStore as any).updateFields(eqId, { functionalTests: base.functionalTests }) } catch {}
        // Media last; may be disallowed by backend
        try { if (Array.isArray(base.images)) await (equipmentStore as any).updateFields(eqId, { images: base.images }) } catch {}
        try { if (Array.isArray(base.attachments)) await (equipmentStore as any).updateFields(eqId, { attachments: base.attachments }) } catch {}
      } catch (e) {
  ...
  try {
    ...
    async function createOneFromRow(row: { tag: string; title?: string; location?: string; status?: string }) {
      ...
      try {
        ...
        try { await equipmentStore.updateFields(createdId, { attributes: base.attributes } as any) } catch {}
        try { await equipmentStore.updateFields(createdId, { components: base.components } as any) } catch {}
        try { await equipmentStore.updateFields(createdId, { checklists: base.checklists } as any) } catch {}
        try { await equipmentStore.updateFields(createdId, { functionalTests: base.functionalTests } as any) } catch {}
        try { if (Array.isArray(base.images) && base.images.length) await equipmentStore.updateFields(createdId, { images: base.images } as any) } catch {}
        try { if (Array.isArray(base.attachments) && base.attachments.length) await equipmentStore.updateFields(createdId, { attachments: base.attachments } as any) } catch {}
        return created
      }
    }
