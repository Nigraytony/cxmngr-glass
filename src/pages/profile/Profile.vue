<template>
  <section class="">
    <div class="w-full mb-4">
      <BreadCrumbs :items="crumbs" title="Profile" />
    </div>
    ...
    <div class="w-full rounded-2xl p-6 bg-white/6 backdrop-blur-xl border border-white/10">
      ...
      <div class="mb-6">
        <div role="tablist" class="relative flex items-center w-full">
          ...
          >
            <!-- Icons per tab -->
            <svg v-if="t === 'Info'" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-white/90" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zM4 20v-1a4 4 0 014-4h8a4 4 0 014 4v1"/></svg>
            <svg v-else-if="t === 'Change Password'" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-white/90" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 11c1.657 0 3-1.567 3-3.5S13.657 4 12 4 9 5.567 9 7.5 10.343 11 12 11zM6 20v-1a4 4 0 014-4h4a4 4 0 014 4v1"/></svg>
            <svg v-else-if="t === 'Avatar'" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-white/90" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M21 19V7a2 2 0 0 0-2-2h-3l-2-2H10L8 5H5a2 2 0 0 0-2 2v10" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><circle cx="12" cy="13" r="3" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
            <svg v-else-if="t === 'Projects'" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-white/90" viewBox="0 0 24 24" fill="none" stroke="currentColor"><rect x="3" y="7" width="18" height="13" rx="2" ry="2" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path d="M16 3v4" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
            <svg v-else-if="t === 'Settings'" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-white/90" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M12 15.5A3.5 3.5 0 1 0 12 8.5a3.5 3.5 0 0 0 0 7z" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 1 1-4 0v-.09a1.65 1.65 0 0 0-1-1.51 1.65 1.65 0 0 0-1.82.33l-.06.06A2 2 0 1 1 2.27 17.4l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 1 1 0-4h.09a1.65 1.65 0 0 0 1.51-1 1.65 1.65 0 0 0-.33-1.82L4.27 3.24A2 2 0 1 1 7.1.41l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V0a2 2 0 1 1 4 0v.09c.12.6.57 1.09 1.17 1.37l.06.03A2 2 0 1 1 19.73 3.6l-.06.06a1.65 1.65 0 0 0-.33 1.82V6c.46.1.9.34 1.2.64.3.3.54.74.64 1.2H21a2 2 0 1 1 0 4h-.09c-.6.12-1.09.57-1.37 1.17l-.03.06z" stroke-width="1" stroke-linecap="round" stroke-linejoin="round"/></svg>
            <span>{{ t }}</span>
          </button>
        </div>
      </div>
          ...
          <form @submit.prevent="save" class="space-y-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input v-model="local.firstName" class="w-full rounded-lg p-2 bg-white/5 border border-white/10 text-white" />
              ...
              <div>
                ...
                <input v-model="local.lastName" class="w-full rounded-lg p-2 bg-white/5 border border-white/10 text-white" />
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <select v-model="local.role" class="w-full rounded-lg p-2 bg-white/5 border border-white/10 text-white">
              ...
              <div>
                ...
                <input v-model="local.contact.company" class="w-full rounded-lg p-2 bg-white/5 border border-white/10 text-white" />
              </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input v-model="local.contact.phone" class="w-full rounded-lg p-2 bg-white/5 border border-white/10 text-white" />
              </div>

              <div>
                <label class="block text-white/80 mb-1">Email</label>
                <input v-model="local.email" type="email" class="w-full rounded-lg p-2 bg-white/5 border border-white/10 text-white" />
            ...
            <div>
              ...
              <input v-model="local.contact.address.street" placeholder="Street" class="w-full rounded-lg mb-2 p-2 bg-white/5 border border-white/10 text-white" />

              <div class="grid grid-cols-1 sm:grid-cols-3 gap-2">
                <input v-model="local.contact.address.city" placeholder="City" class="w-full rounded-lg p-2 bg-white/5 border border-white/10 text-white" />
                <input v-model="local.contact.address.state" placeholder="State" class="w-full rounded-lg p-2 bg-white/5 border border-white/10 text-white" />
                <input v-model="local.contact.address.zip" placeholder="Zip" class="w-full rounded-lg p-2 bg-white/5 border border-white/10 text-white" />
              </div>

              <input v-model="local.contact.address.country" placeholder="Country" class="w-full rounded-lg mt-2 p-2 bg-white/5 border border-white/10 text-white" />
            </div>

            <div>
              <label class="block text-white/80 mb-1">Bio</label>
              <textarea v-model="local.contact.bio" rows="3" class="w-full rounded-lg p-2 bg-white/5 border border-white/10 text-white"></textarea>
            </div>
            ...
            <div class="flex items-center gap-3">
              <button
                ...
                aria-busy="saving"
              >
                <svg v-if="saving" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 mr-2 animate-spin text-white/90" viewBox="0 0 24 24" fill="none">
                  <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                  <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z"></path>
                </svg>
                Save
              </button>
              <button type="button" @click="reset" class="px-4 py-2 rounded-lg bg-white/6 hover:bg-white/10 text-white">Cancel</button>
              <div v-if="status" class="ml-auto text-white/80">{{ status }}</div>
            </div>
          </form>
        </div>

        <div v-else-if="currentTab === 'Change Password'" class="space-y-4">
          <p class="text-white/80">Change your account password.</p>
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label class="block text-white/80 mb-1">Email</label>
              <input :value="auth.user?.email || ''" disabled class="w-full rounded-lg p-2 bg-white/5 border border-white/10 text-white/50" />
            </div>
            <div>
              <label class="block text-white/80 mb-1">Current password</label>
              <input v-model="pwd.current" type="password" class="w-full rounded-lg p-2 bg-white/5 border border-white/10 text-white" />
              <p v-if="pwdErrors.current" class="text-sm text-red-400 mt-1">{{ pwdErrors.current }}</p>
            </div>
            <div>
              <label class="block text-white/80 mb-1">New password</label>
              <input v-model="pwd.new" type="password" class="w-full rounded-lg p-2 bg-white/5 border border-white/10 text-white" />
              <p v-if="pwdErrors.new" class="text-sm text-red-400 mt-1">{{ pwdErrors.new }}</p>
            </div>
            <div>
              <label class="block text-white/80 mb-1">Confirm new password</label>
              <input v-model="pwd.confirm" type="password" class="w-full rounded-lg p-2 bg-white/5 border border-white/10 text-white" />
              <p v-if="pwdErrors.confirm" class="text-sm text-red-400 mt-1">{{ pwdErrors.confirm }}</p>
            </div>
          </div>
          <div class="flex items-center gap-3">
            <button :disabled="pwdSaving" @click="changePasswordHandler" class="px-4 py-2 rounded-lg bg-white/6 text-white" :class="pwdSaving ? 'opacity-60 cursor-not-allowed' : 'hover:bg-white/10'">Update password</button>
            <div v-if="pwdStatus" class="ml-4 text-white/80">{{ pwdStatus }}</div>
          </div>
        </div>

        <div v-else-if="currentTab === 'Avatar'" class="space-y-4">
          <p class="text-white/80">Upload or change your avatar.</p>

          <div class="flex items-start gap-6">
            <div class="w-28 h-28 rounded-full overflow-hidden bg-white/6 flex items-center justify-center">
              <img v-if="preview || local.avatar" :src="preview || local.avatar" alt="avatar preview" class="w-full h-full object-cover" />
              <div v-else class="text-white/70">No avatar</div>
            </div>

            <div class="flex-1">
              <label class="block text-white/80 mb-2">Choose image</label>
              <input ref="fileInput" type="file" accept="image/*" @change="onFileChange" class="text-white/70" />

              <div class="mt-4 flex items-center gap-3">
                <button :disabled="!preview" @click="startCrop" class="px-4 py-2 rounded-lg bg-white/6 hover:bg-white/10 text-white" :class="!preview ? 'opacity-50 cursor-not-allowed' : ''">Crop</button>
                <button v-if="isCropping" @click="applyCrop" class="px-4 py-2 rounded-lg bg-white/6 hover:bg-white/10 text-white">Apply Crop</button>
                <button v-if="isCropping" @click="cancelCrop" class="px-4 py-2 rounded-lg bg-white/6 hover:bg-white/10 text-white">Cancel Crop</button>
                <button v-else :disabled="!preview" @click="uploadAvatar" class="px-4 py-2 rounded-lg bg-white/6 hover:bg-white/10 text-white" :class="!preview ? 'opacity-50 cursor-not-allowed' : ''">Upload</button>
                <button :disabled="!local.avatar && !preview" @click="removeAvatar" class="px-4 py-2 rounded-lg bg-white/6 hover:bg-white/10 text-white">Remove</button>
                <div v-if="avatarStatus" class="ml-4 text-white/80">{{ avatarStatus }}</div>
              </div>
            </div>
          </div>

          <!-- Cropping area -->
          <div v-if="preview && isCropping" class="mt-6">
            <div class="flex flex-col md:flex-row gap-4">
              <div class="relative bg-black/10 p-2 rounded">
                <div ref="previewContainer" class="relative" style="width:320px;height:320px;overflow:hidden;">
                  <img ref="cropImage" :src="preview" alt="to crop" class="w-full h-full object-contain" />
              ...
              <div class="flex-1">
                ...
                <input type="range" min="50" :max="320" v-model.number="crop.sizePx" class="w-full" />
                <div class="mt-4 text-white/80">Drag the crop square to reposition. Click Apply Crop when ready.</div>
              </div>
            </div>
          </div>
        </div>
