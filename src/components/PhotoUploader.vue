  ...
  <div>
    ...
    >
      <div class="flex items-center gap-2">
        <label :for="inputId" class="px-3 py-2 rounded-md bg-white/20 border border-white/30 hover:bg-white/30 cursor-pointer select-none" :class="(uploadingNow || disabled) ? 'opacity-60 cursor-not-allowed' : ''">{{ buttonLabel }}</label>
        <input :id="inputId" type="file" :accept="accept" :multiple="multiple" @change="onFiles" class="sr-only" :disabled="uploadingNow || disabled" />
      </div>
      <div v-if="remainingInfo" class="text-xs text-white/60">{{ remainingInfo }}</div>
    </div>

    <div v-if="notice" class="mt-2 text-xs text-white/70">{{ notice }}</div>
