      ...
      <div class="rounded-md border border-white/10 bg-white/5">
        ...
        >
          <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <!-- Add new signature card (single) -->
            <div v-if="sigList.length < 6" class="p-3 rounded bg-white/6 border border-white/10">
              <div class="mb-2">
                <div class="font-medium text-white/90">New signature</div>
                <div class="text-sm text-white/70 mt-1">Add title and name, then capture signature</div>
              </div>
              <div class="mb-2">
                <input v-model="newSig.title" placeholder="Title" class="w-full px-2 py-1 rounded bg-white/10 border border-white/20 text-white/90 mb-2">
                <input v-model="newSig.person" placeholder="Name" class="w-full px-2 py-1 rounded bg-white/10 border border-white/20 text-white/90">
              </div>
              <div class="mb-2">
                <SignaturePad v-model="newSig.block" />
              </div>
              <div class="mt-2 flex items-center justify-between">
                <div class="text-sm text-white/60">Date: {{ formatDateTime(newSig.date) }}</div>
                <div class="flex items-center gap-2">
                  <button class="px-3 py-1 rounded-md bg-white/10" @click="clearNewSig">Clear</button>
                  <button class="px-3 py-1 rounded-md bg-red-500/20 text-red-200" @click="removeNewSig">Remove</button>
                  <button class="px-3 py-1 rounded-md bg-white/20" @click="addSignature">Save</button>
                </div>
              </div>
            </div>
          </div>
...
function clearNewSig() {
  newSig.block = ''
  newSig.date = new Date().toISOString()
}

function removeNewSig() {
  newSig.title = ''
  newSig.person = ''
  newSig.block = ''
  newSig.date = new Date().toISOString()
