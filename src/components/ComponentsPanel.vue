<template>
  <div class="space-y-3">
    <div class="flex items-center justify-between">
      <div class="text-white/80">Define the components that make up this item.</div>
      <button @click="startNewComponent" class="px-3 py-2 rounded-md bg-white/10 border border-white/20 hover:bg-white/15">Add Component</button>
    </div>

    <div v-if="!local.length" class="text-white/60">No components yet.</div>
      ...
      <table class="min-w-full text-sm">
        <thead class="bg-white/5 text-white/70">
          <tr>
            ...
            <th class="text-left px-3 py-2">Key attributes</th>
        ...
        <tbody>
          ...
          >
            <td class="px-2 py-2 align-middle">
              <span class="h-7 w-7 grid place-items-center rounded-md bg-white/10 border border-white/20 text-white/70 cursor-grab active:cursor-grabbing select-none" title="Drag to reorder" aria-label="Drag to reorder">
            ...
            <td class="px-3 py-2">
              <span v-if="attrSummary(c.attributes)" class="text-white/70">{{ attrSummary(c.attributes) }}</span>
            ...
            <td class="px-3 py-2 text-right">
              ...
              >
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" class="w-4 h-4">
                  <path d="M12 20h9" stroke-width="1.5" stroke-linecap="round"/>
                  <path d="M16.5 3.5l4 4-10 10H6.5v-4.5l10-10z" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
              </button>
              <button
    ...
    <Modal v-model="showCompModal">
      <template #header>
        <h3 class="text-lg font-semibold">{{ compDraft.id ? 'Edit Component' : 'Add Component' }}</h3>
      </template>
      <div class="space-y-4">
        <div class="grid md:grid-cols-3 gap-3">
          <div>
            <label class="block text-sm text-white/70">Tag</label>
            <input v-model="compDraft.tag" placeholder="SF-01-FAN" class="w-full px-3 py-2 rounded-md bg-white/10 border border-white/20 placeholder-gray-400" />
          </div>
          <div>
            <label class="block text-sm text-white/70">Type</label>
            <input v-model="compDraft.type" placeholder="fan, damper, sensor, vfd, ..." class="w-full px-3 py-2 rounded-md bg-white/10 border border-white/20 placeholder-gray-400" />
          </div>
          <div>
            <label class="block text-sm text-white/70">Title/Label</label>
            <input v-model="compDraft.title" placeholder="Supply Fan, OA Damper, ..." class="w-full px-3 py-2 rounded-md bg-white/10 border border-white/20 placeholder-gray-400" />
          </div>
          <div>
            <label class="block text-sm text-white/70">Status</label>
            <div class="relative">
              <select v-model="compDraft.status" class="w-full px-3 pr-10 py-2 rounded-md bg-white/10 border border-white/20 appearance-none">
                <option :value="''">—</option>
        ...
        <div>
          <div class="text-sm text-white/80 mb-1">Attributes</div>
          <div v-if="compAttrList.length === 0" class="text-white/60">No attributes yet.</div>
          <ul v-else class="divide-y divide-white/10">
            <li v-for="(a, i) in compAttrList" :key="i" class="py-2 flex items-center gap-2">
              <template v-if="compAttrEditingIndex === i">
                <input v-model="compAttrKey" class="flex-1 min-w-0 px-2 py-1 rounded-md bg-white/10 border border-white/20 placeholder-gray-400" placeholder="Key" />
                <input v-model="compAttrValue" class="flex-1 min-w-0 px-2 py-1 rounded-md bg-white/10 border border-white/20 placeholder-gray-400" placeholder="Value" />
                <button
        ...
        <div>
          ...
          <textarea v-model="compDraft.notes" rows="3" class="w-full px-3 py-2 rounded-md bg-white/10 border border-white/20 placeholder-gray-400" placeholder="Notes about this component..."></textarea>
          <div v-if="compDraftIssueLinks.length" class="mt-2">
            <div class="text-sm text-white/80 mb-1">Linked issues</div>
            <ul class="list-disc list-inside text-white/80 space-y-0.5">
              <li v-for="it in compDraftIssueLinks" :key="it.id" class="truncate">
      ...
      <template #footer>
        <div class="flex items-center gap-2">
          <button @click="saveComponent" :disabled="!compDraft.type?.trim()" class="px-3 py-2 rounded-md bg-emerald-600/30 border border-emerald-400/40 hover:bg-emerald-600/40 disabled:opacity-60">Save Component</button>
          <button v-if="canCreateIssues" @click="openCompIssueModal" class="px-3 py-2 rounded-md bg-indigo-500/20 border border-indigo-400/60 text-indigo-100 hover:bg-indigo-500/35">Add Issue</button>
          <button @click="cancelEditComponent" class="px-3 py-2 rounded-md bg-white/10 border border-white/20 hover:bg-white/15">Cancel</button>
        </div>
      </template>
    </Modal>
    ...
    <Modal v-model="showCompIssueModal">
      <template #header>
        <h3 class="text-lg font-semibold">Add Issue for Component</h3>
      </template>
      <IssueForm v-model="compIssueDraft" />
      <template #footer>
        <div class="flex gap-2">
          <button @click="createCompIssue" class="px-4 py-2 rounded-lg bg-white/6 hover:bg-white/10 text-white">Create</button>
          <button @click="showCompIssueModal = false" class="px-4 py-2 rounded-lg bg-white/6 hover:bg-white/10 text-white">Cancel</button>
        </div>
      </template>
    </Modal>
  </div>
...
function onCompDragStart(c: any, e: DragEvent) {
  ...
  if (e && e.dataTransfer) { try { e.dataTransfer.setData('text/plain', 'component') } catch {}; e.dataTransfer.effectAllowed = 'move' }
}
